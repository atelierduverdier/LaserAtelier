...
- **Marquage de motif (plat ou courbe)** : grave un motif filaire à plat (motif 2D seul) ou en suivant le relief d'un modèle 3D (sonde par tessellation, ou interpolation), avec préréglages matériau, aperçu du trajet directement dans la vue 3D, et **styles de trait** (plein / tirets / pointillé / vague défocus / **défocus point élargi** — ce dernier grave le motif au-dessus du foyer pour noircir un remplissage en un passage, l'équivalent du remplissage Défocus des Hachures 2D appliqué au motif projeté ; tous les styles suivent le relief comme le trait plein).
...
- **Test rampe puissance/vitesse (lignes)** : grave de longues lignes, **une par vitesse**, chacune parcourue avec une puissance qui **monte progressivement** de gauche (min) à droite (max). On lit d'un coup, à chaque vitesse, à partir de quelle puissance le trait commence à marquer et où il sature — le complément **continu** de la grille de cellules discrètes. Option **rampe de hauteur (Z)** : la hauteur du bec monte *aussi* le long de chaque ligne, de la focale (gauche) à une hauteur de fin (droite), en même temps que la puissance — pour tester à chaque vitesse l'effet combiné puissance croissante + défocus croissant. Étiquettes vitesse (F) à gauche et bornes de puissance (Smin/Smax) sous la première ligne.
