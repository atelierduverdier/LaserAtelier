...
<p align="center"><img src="resources/icons/chapeau.svg" alt="" width="56"><br><sub><b>v2.99.46</b> — Le petit chapeau en coin de chaque icône est la signature de l'<a href="https://atelierduverdier.fr">Atelier du Verdier</a>.<br>© Atelier du Verdier — licence <a href="LICENSE">LGPL-2.1-or-later</a>.</sub></p>
...
> Ce README décrit l'atelier **tel qu'il fonctionne**. L'historique — ce qui n'allait pas, ce que le bois a dit, ce qui a été essayé et écarté — vit dans le manuel, chapitre 13 «&nbsp;[Journal de l'atelier](docs/Manuel-LaserAtelier.pdf)&nbsp;», 161 entrées avec leurs mesures.

Les modes sont regroupés par thème dans **neuf barres d'outils nommées** (Découverte, Calibrer le laser, Ajouter un matériau, Dessins, Gravure à plat, Sur surface 3D, Découpe, Assemblage, Référence et réglages) et dans un menu unique. Une barre nommée se déplace, se masque et se replie séparément — les séparateurs d'une barre unique ne se voyaient pas ; chacune reçoit en plus un fond légèrement teinté, en renfort du nom et jamais à sa place. Les icônes **suivent le thème de FreeCAD** : sur un fond sombre, l'encre ardoise de la charte serait invisible (1,4:1 de contraste sur le thème PrintNC), l'atelier fabrique donc au démarrage un jeu à l'encre claire (11:1) — un redémarrage après un changement de thème. Seul le **chapeau** y échappe&nbsp;: c'est la signature de l'atelier, pas un trait de dessin, et ses deux reflets blancs suffisent à la faire lire sur fond sombre — éclaircie, elle deviendrait un melon blanc. Le **Guide rapide** (première icône, livre ouvert) résume le flux de travail (calibrer → tester → motif → G-code → cadrage → graver) et « quel mode pour quoi ? » — le point d'entrée pour découvrir l'atelier. **Workflow type** : créer/charger le tracé (SVG, ShapeString, sketch) → le placer en X/Y par rapport au futur zéro pièce → le **projeter** sur la pièce (contrôle visuel : le tracé est posé sur la surface, plate ou 3D) → graver avec **Marquage** (traits, sélectionner le projeté) ou **Gravure remplie** (noir plein, sélectionner la forme fermée *source*). La hauteur Z du document est visuelle : le G-code suppose toujours zéro Z machine sur la surface gravée, et un job = une seule surface de référence. Chaque panneau ouvre sur un résumé court avec un bouton « En savoir plus » (détails repliés) et, pour les concepts clés, un petit schéma explicatif (cône de défocus, axes de la grille, rampe, projection…).
...
- **Assistance d'air** : choisir **M7** (brouillard) ou **M8** (arrosage) dans les Préférences,
  selon la sortie sur laquelle la pompe est **câblée** — par profil laser. `M9` referme, quelle que
  soit celle qui a ouvert. ⚠️ Les deux sont des broches HAL différentes : celle qui n'est pas câblée
  ne fait rien, et le fichier reste parfaitement valide — il grave simplement sans air.
