...
| Module | Lines | Role |
|---|---|---|
| `laser_core.py` | ~14 900 | ALL geometry + G-code logic. **No Qt.** Generators, defocus model, fonts, persistence, `_USER_SETTINGS`. The layer you unit-test headless. |
| `task_panels.py` | ~20 800 | One `TaskPanel*` per mode (PySide6). Pure UI: builds the form, reads widgets, calls `core.*`. No geometry math. |
| `laser_jobs.py` | ~900 | Tree "Job" objects — bookmarks, not a second source of truth. |
| `svg_import.py` | ~1 100 | Standalone SVG→geometry parser, no Draft/DXF detour. |
...
| `icones.py` | ~280 | Icon set matched to the Qt theme. On a dark palette it *generates* a light-ink copy of `resources/icons/` in a cache dir; also serves the ink colours panels paint text with. |
| `commands.py` | ~600 | One `*Command` per mode + `register_commands()`. |
| `InitGui.py` | ~250 | The `Workbench` class: toolbar/menu order, lazy imports. |

`laser_core.py` is organised into banner-commented sections, one per mode. Keep that shape.

**`polices_monotrait/`** holds the 45 mono-line font DATA modules (`hershey_font*.py`,
...
other installed workbench. Forty-five exposed names became one. Generated by
